...
fungi <- readPNG(here("pngs", "fungi.png"))
fungi <- rasterGrob(fungi)

fly <- readPNG(here("pngs","fly.png"))
fly <- rasterGrob(fly)

algae <- readPNG(here("pngs", "chlorella.png"))
algae <- rasterGrob(algae)

cow <- readPNG(here("pngs", "cow_2.png"))
cow <- rasterGrob(cow)

hen <- readPNG(here("pngs", "chicken_2.png"))
hen <- rasterGrob(hen)

beans <- readPNG(here("pngs", "seeds.png"))
beans <- rasterGrob(beans)

fish <- readPNG(here("pngs/tuna_2.png"))
fish <- rasterGrob(fish)

meat <- readPNG(here("pngs/meat.png"))
meat <- rasterGrob(meat)

fish2 <- readPNG(here("pngs/tilapia.png"))
...
ggplot(data = impact_indicators %>% 
         ...
         mutate(label = paste0(food," (", round(ghg_50g_proetin, digits = 1), ")")) %>% 
          mutate(label_kg = paste0(food," (", round(ghg_50g_proetin, digits = 1)*20, ")")),
       
       aes(x=ghg_50g_proetin, y=y))+
  geom_line(aes(colour=ghg_50g_proetin*20), size=2)+
  geom_point(size=1.2)+
  scale_colour_gradient(low = "lemonchiffon", high = "firebrick", breaks = )+
  scale_x_continuous(limits = c(-1, 13))+
  scale_y_continuous(limits = c(0.4, 0.58))+
  geom_text(mapping = aes(x=x, y=shuffle, label=label_kg, hjust=hjust), angle=45,size=2, nudge_x = -0.05) +
  ...
  labs(colour = bquote(GWP[100]~(kg~CO[2]~eq.~kg~protein^-1)))+
  ...
  ggsave(here("figs", "impacts_line.jpg"), device = "jpg", dpi=600, width = 9, height = 6, units = "cm")+
  ggsave(here("figs", "impacts_line.pdf"), device = "pdf", dpi=600, width = 9, height = 6, units = "cm")+
  ggsave(here("figs", "impacts_line.tiff"), device = "tiff", dpi=600, width = 9, height = 6, units = "cm")
...
(diffuse_substitution <-  data %>% filter(type %in% c("partial_disadopt_byproduct", "diffuse_disadopt", "full_adopt")) %>% 
   ...
   guides(colour = FALSE)+
   ggsave(here("scripts/explore/full_adopt_diffuse_sub.jpg"), device="jpg", dpi=300, width=9, height = 5, units = "cm")
)
...
impact_indicator_change_diffuse <- bind_rows(
 ...
 theme(axis.text.x = element_blank(),
  ...
  #guides(colour=FALSE)+
  ggsave(here("scripts/explore/impact_indicator_change_diffuse.jpg"), device="jpg", dpi=300, width=9, height = 5, units = "cm")
...
substitution_impact_displaced <- ggplot(data=impact_indicator_change_displaced, aes(x=x, y=Relative_impact_indicator, colour=sub_type))+
 ...
 ggarrange(diffuse_substitution, substitution_impact_diffuse,
           ...
           ncol=2,
           align = "hv")+
  ggsave(here("scripts/explore/Figure 3 - the role of substitution.jpg"), device = "jpg", dpi = 600, width = 11.4, height = 9, units="cm")+
   ggsave(here("scripts/explore/Figure 3 - the role of substitution.tiff"),device = "tiff", dpi = 600,  width = 11.4, height = 9, units="cm")+
   ggsave(here("scripts/explore/Figure 3 - the role of substitution.pdf"), device = "pdf", dpi=600, width = 11.4, height = 9, units="cm")
...
#Sensitivity analysis for nutrients, servings, and minerals

Edible quantity
```{r}

(diffuse_substitution_edible <-  data %>% filter(type %in% c("partial_disadopt_byproduct", "diffuse_disadopt", "full_adopt")) %>% 
   mutate(curve = case_when(curve == "Adoption"  ~ "New food",
                            curve == "Disadoption" & type == "diffuse_disadopt" ~ "Target products",
                            curve == "Disadoption" & type == "partial_disadopt_byproduct" ~ "Non-target products")) %>%
   mutate(curve = factor(curve, levels = c( "Target products", "Non-target products", "New food"))) %>% 
   ...
   theme(
     ...
     axis.title = element_text(size=6),
        axis.text.y = element_text(size=6))+
   labs(y=bquote(Consumption~(kg~capita^-1~year^-1)), x= expression(Time%->%""), subtitle = "Full adoption, partial substitution of beef")+
   scale_color_manual(values=lacroix_palette("PassionFruit", n=3))+
   ...
   annotate("text", x=-4.1, y=0.12*20, label="Cultured\nmeat", size=1.5, hjust=0, colour = "black")+
   guides(colour = FALSE)+
   ggsave(here("scripts/explore/full_adopt_diffuse_sub_edible_portion.jpg"), device="jpg", dpi=300, width=9, height = 5, units = "cm")
)


impact_indicator_change_diffuse_protein <- impact_indicator_change_diffuse %>% 
  mutate(metric = "Protein quantity")
 

impact_indicator_change_diffuse_edible_portion <- bind_rows(
  myco_ghgs <- data %>% filter(type %in% c("partial_disadopt_byproduct", "diffuse_disadopt", "full_adopt")) %>% 
    mutate(curve = case_when(curve == "Adoption" ~ "Mycoprotein",
                             curve == "Disadoption" &  type == "diffuse_disadopt" ~ "Beef",
                             curve == "Disadoption" & type == "partial_disadopt_byproduct" ~ "Pulses")) %>% 
    mutate(impact_indicator = case_when(curve=="Mycoprotein" ~ 9.4,
                                        curve=="Pulses" ~ 0.9,
                                        curve == "Beef" ~ 123.7)) %>% 
    #mutate(impact_indicator2 = impact_indicator*20) %>% 
    mutate(true_value = value*20) %>% 
  ...
  chlorella_substitution <- data %>% filter(type %in% c("partial_disadopt_byproduct", "diffuse_disadopt", "full_adopt")) %>% 
    mutate(curve = case_when(curve == "Adoption" ~ "Microalgae",
                             curve == "Disadoption" &  type == "diffuse_disadopt" ~ "Beef",
                             curve == "Disadoption" & type == "partial_disadopt_byproduct" ~ "Pulses")) %>% 
    mutate(impact_indicator = case_when(curve=="Microalgae" ~ 69.5,
                                        curve=="Pulses" ~ 0.9,
                                        curve == "Beef" ~ 123.7)) %>% 
    mutate(true_value = value*20) %>% 
    mutate(Relative_stressor = true_value*impact_indicator) %>% 
  ...
  BSF_substitution <- data %>%  filter(type %in% c("partial_disadopt_byproduct", "diffuse_disadopt", "full_adopt")) %>% 
    mutate(curve = case_when(curve == "Adoption" ~ "Insects",
                             curve == "Disadoption" &  type == "diffuse_disadopt" ~ "Beef",
                             curve == "Disadoption" & type == "partial_disadopt_byproduct" ~ "Pulses")) %>% 
    mutate(impact_indicator = case_when(curve=="Insects" ~ 6.6,
                                        curve=="Pulses" ~ 0.9,
                                        curve == "Beef" ~ 123.7)) %>% 
    mutate(true_value = value*20) %>% 
    mutate(Relative_stressor = true_value*impact_indicator) %>% 
  ...
  tilapia_substitution <- data %>%  filter(type %in% c("partial_disadopt_byproduct", "diffuse_disadopt", "full_adopt")) %>% 
    mutate(curve = case_when(curve == "Adoption" ~ "Tilapia",
                             curve == "Disadoption" &  type == "diffuse_disadopt" ~ "Beef",
                             curve == "Disadoption" & type == "partial_disadopt_byproduct" ~ "Pulses")) %>% 
    mutate(impact_indicator = case_when(curve=="Tilapia" ~ 58.5,
                                        curve=="Pulses" ~ 0.9,
                                        curve == "Beef" ~ 123.7)) %>% 
    mutate(true_value = value*20) %>% 
    mutate(Relative_stressor = true_value*impact_indicator) %>% 
  ...
  cultmeat_substitution <- data %>%  filter(type %in% c("partial_disadopt_byproduct", "diffuse_disadopt", "full_adopt")) %>% 
    mutate(curve = case_when(curve == "Adoption" ~ "Cultured meat",
                             curve == "Disadoption" &  type == "diffuse_disadopt" ~ "Beef",
                             curve == "Disadoption" & type == "partial_disadopt_byproduct" ~ "Pulses")) %>% 
    mutate(impact_indicator = case_when(curve=="Cultured meat" ~ 14.6,
                                        curve=="Pulses" ~ 0.9,
                                        curve == "Beef" ~ 123.7)) %>% 
    mutate(true_value = value*20) %>% 
    mutate(Relative_stressor = true_value*impact_indicator) %>% 
  ...
  unnest() %>% 
  mutate(sub_type = factor(sub_type, levels = c("Chlorella microalgae", "Tilapia (farmed)", "Cultured meat","Mycoprotein", "Black soldier fly" ))) %>% 
  mutate(metric = "Edible quantity")


impact_indicator_change_sensitvity <- bind_rows(impact_indicator_change_diffuse_protein, impact_indicator_change_diffuse_edible_portion)




 substitution_impact_sensitivity <- ggplot(data=impact_indicator_change_sensitvity, aes(x=x, y=Relative_impact_indicator, colour=sub_type, linetype=metric))+
   geom_hline(yintercept = 0, linetype=2, size=0.25, colour = "grey70")+
  geom_line(size=0.9)+
  theme_pubr()+
 theme(axis.text.x = element_blank(),
        #axis.title.x = element_blank(),
        axis.ticks.x = element_blank(),
        legend.position = c(0.53,0.18),
       legend.box = "horizontal",
       legend.margin = margin(t=-0.05,r=-0.1,b=-0.05,l=-0.15, unit="cm"),
       legend.box.margin = margin(t=-0.3,r=0,b=0,l=0, unit="cm"),
        text = element_text(size=6),
        axis.title.y = element_text(size=6),
       legend.background = element_rect(fill="transparent", colour="transparent"),
        legend.key.size = unit(0.4, "cm"),
       legend.key.height = unit(0.2, "cm"))+
  labs(y=bquote(Delta~GWP[100]~(kg~CO[2]~eq.)), x= expression(Time%->%""),colour = "Protein substitute", linetype = "Comparison metric", subtitle = " ")+
  scale_color_manual(values=lacroix_palette("Lemon", n=5))+
   scale_y_continuous(limits=c(-120*20,70*20))+
  # geom_hline(yintercept = c(10.01,10.29, 15.65), linetype=2, colour="grey70")+
  # annotate("text", x=-5, y=16.2, label="Chlorella = 15.65", size=2, hjust=0 )+
  # annotate("text", x=-5, y=11, label="Mycoprotein = 10.29", size=2, hjust=0)+
  # annotate("text", x=-5, y=9.2, label="Black soldier fly = 10.01", size=2, hjust=0)+
  ggsave(here("scripts/explore/impact_indicator_change_diffuse_sensitivity.jpg"), device="jpg", dpi=300, width=9, height = 5, units = "cm")


 
 Figure_s1 <- ggarrange(diffuse_substitution_edible,
                        substitution_impact_sensitivity,
                        labels = c("(a)", "(b)"), 
                        font.label = list(size=7),
                        label.y = 1,
                        label.x = 0.05,
  ...
  ggsave(here("figs/Figure S1 - protein versus quantity.jpg"), device = "jpg", dpi=600, width=11.4, height = 4.5, units = "cm")
